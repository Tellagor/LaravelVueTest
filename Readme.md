# Интеграция с Яндекс.Картами — парсинг отзывов и рейтинга
...
- Итоговая страница с реальными данными
...
- Ротация прокси/User-Agent (Причины описаны в блоке Ответы по дополнительным требованиям)

## Ответы по дополнительным требованиям (1-5)
1. Парсер бросает SourceLayoutChangedException, если на странице не находится csrfToken, блок aggregateRating или ожидаемая структура JSON-ответа fetchReviews. Ошибка попадает в parsing_runs.error и organizations.last_error - статус организации становится failed с текстом причины, а не молча возвращает пустоту.

2. Обоснование подхода к парсингу:
    См. блоки "Как работает парсер" и "Неудавшийся вариант" 

3. Парсинг идёт через очередь Laravel (ParseOrganizationReviews Job), не блокирует HTTP-запрос сохранения ссылки. Ретраи с нарастающим бэкоффом при rate-limit (1→5→15 мин), до 3 попыток. Прогресс сейчас грубый (только статус), без счётчика "N из M отзывов" — это упрощение, полноценный прогресс-бар не реализовывал.

4. Анти-бан - реализована пауза между запросами, реалистичные заголовки. Не сделана ротация проксей, не делал этого, так как посчитал не нужным для тестового, вот как я бы сделал это при необходимости: 
    ...
    - Ротация User-Agent - не сделана. Сейчас используется один захардкоженный UA всегда. Если делать смену UA, то можно собрать один пул реалистичных строк UA и выбор случайного UA на каждую сессию скрапинга. 

    - Бэкофф на уровне очереди - при получении 429/403 вся задача откладывается с нарастающей задержкой 1 -> 5 -> 15 минут, а не просто падает. Это отдельная защита поверх паузы между страницами. 

5. Отзывы сохраняются через upsert по паре (organization_id, external_id) - повторный парсинг обновляет существующие записи, а не создаёт дубли. Таблица organization_snapshots хранит рейтинг/счётчики на момент каждого успешного парсинга - можно построить "было -> стало" между запусками.

4. Обработка "нас забанили" как отдельного случая - сделана. 

## Запуск через Docker
1) создать .env в корне (переменные для docker-compose: пароли БД, FRONTEND_BACKEND_URL)
2) создать backend/.env (со значениями под docker-сеть: DB_HOST=mysql, а не 127.0.0.1, домены под
3) команды — docker compose up -d --build, php artisan key:generate, php artisan migrate --seed
4) куда зайти (localhost — фронт, localhost:8080 — бэк) и креды входа
    - Предупреждение про первую сборку — компиляция PHP-расширений (intl/icu) может занять несколько минут при первом docker compose up --build, это нормально, не зависание.
    - что при деплое на облачный VPS/хостинг /fetchReviews может блокироваться Яндексом по IP-репутации датацентра (так показала практика при дропе на сервера TimeWeb'а) Дабы этого избежать достаточно подключить прокси, о чем уже говорилось намного выше.
...
Если бы было больше времени: докрутил бы прогресс-бар парсинга по числу отзывов, добавил бы ротацию User-Agent (это дёшево и просто), причесал бы CSS.
